...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write(self.style.MIGRATE_HEADING(f"Creating Default Scenario: {target_id}"))

        try:
            # 서비스 호출
            scenario, summary = create_scenario_from_base(target_id)

            self.stdout.write(self.style.SUCCESS(msg.SCENARIO_CREATE_SUCCESS.format(scenario_id=target_id)))
