...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write(self.style.MIGRATE_HEADING(msg.DB_COMMENT_UPDATE_START.format(vendor=vendor)))

        # SQLite는 미지원
        if vendor == 'sqlite':
            self.stdout.write(self.style.ERROR(msg.DB_COMMENT_NOT_SUPPORTED.format(vendor=vendor)))
            return
        ...
        with connection.cursor() as cursor:
            for model in app_config.get_models():
                    ...
                    try:
                        ...
                        if sql:
                            ...
                            self.stdout.write(self.style.SUCCESS(
                                msg.DB_TABLE_COMMENT_SUCCESS.format(table=db_table, comment=table_verbose)))
                    except Exception as e:
                        self.stdout.write(self.style.ERROR(msg.DB_COMMENT_FAIL.format(target=db_table, error=str(e))))
                ...
                for field in model._meta.fields:
                    ...
                    if not column_verbose or column_name == 'id':  # ID는 보통 생략하거나 필요 시 포함
                        continue
                    ...
                    try:
                        ...
                        self.stdout.write(self.style.ERROR(msg.DB_COMMENT_FAIL.format(target=target_str, error=str(e))))
