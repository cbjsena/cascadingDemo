...
class TestLongRangeService:
    ...
    def test_lrs_svc_head_tail_y_logic(self, lrs_service, pf_complex_data):
        ...
        rows = list(ProformaSchedule.objects.filter(
        ...
        assert len(expanded) == 4, "Head Y는 생성하고 Tail Y는 생성하지 않아 총 4개여야 합니다."
    ...
    def test_lrs_svc_date_continuity(self, lrs_service, sample_schedule, user):
        ...
        # Then
        lrs_qs = LongRangeSchedule.objects.filter(vessel_code="V_DT_TEST").order_by("etb")
        ...
        assert abs(diff.days - 14) <= 1, f"날짜 차이는 14일이어야 합니다. 실제: {diff.days}"
...
class TestLongRangeServiceMissingScenarios:
    ...
    def test_lrs_svc_002_validation_duration_zero(self, lrs_service, pf_complex_data, user):
        ...
        assert "Invalid Proforma Duration" in str(exc_info.value) or "0" in str(exc_info.value)
        # 데이터가 생성되지 않아야 함
    ...
    def test_lrs_svc_mid_y_virtual_port(self, lrs_service, pf_complex_data, user):
        ...
        ProformaSchedule.objects.create(
            scenario=pf_complex_data, lane_code="LANE_MID", proforma_name="PF_MID_Y",
            effective_from_date=timezone.now(),terminal_code ='PORT_A01',
            declared_count=2, duration=10.0,
            etb_day_code="SUN", etb_day_time="0800", etb_day_number=0,
            port_code="PORT_A", calling_port_seq=1, direction="E", turn_port_info_code="N",
            created_by=user, updated_by=user
        )
        ProformaSchedule.objects.create(  # 중간 Y 포트 (환적항 등)
            scenario=pf_complex_data, lane_code="LANE_MID", proforma_name="PF_MID_Y",
            effective_from_date=timezone.now(), terminal_code='PORT_B01',
            declared_count=2, duration=10.0,
            etb_day_code="MON", etb_day_time="0800", etb_day_number=0,
            port_code="PORT_B", calling_port_seq=2, direction="E", turn_port_info_code="Y",
            created_by=user, updated_by=user
        )
        ProformaSchedule.objects.create(
            scenario=pf_complex_data, lane_code="LANE_MID", proforma_name="PF_MID_Y",
            ...
            declared_count=2, duration=10.0,
            calling_port_seq=3, direction="W", turn_port_info_code="N",
            etb_day_code="SAT", etb_day_time="0800", etb_day_number=0,
            port_code="PORT_C", terminal_code='PORT_C01',
            created_by=user, updated_by=user
        )
        ...
        qdict.update({
            ...
            "apply_start_date": start_date.strftime("%Y-%m-%d"),
            "apply_end_date": (start_date + timedelta(days=15)).strftime("%Y-%m-%d"),
        })
        ...
        mid_ports = LongRangeSchedule.objects.filter(vessel_code="V_MID", port_code="PORT_B", voyage_number="0001")
